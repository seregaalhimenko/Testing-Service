...
from app.detail_serializers import ThemeDetailSerializer
...
class ThemeTests(APITestCase):
    ...
    def setUp(self) -> None:
        ...
        self.client.post("http://0.0.0.0:8000/auth/users/", data, format='json')
    ...
    def test_create_Theme(self):
        ...
        self.assertEqual(Theme.objects.get().name, 'test_theme') # response.headers["Location"] по договоренности rest (вроде) id созданной модели должен быть в location и не должен возвращать нечего...поправим :)
        self.theme_id = response.data['id']
        

    def test_show_Theme(self):
        create_theme_response = self.create_theme()
        url = reverse('theme_detail', kwargs={'pk': create_theme_response.data['id']})
        response: Response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = ThemeDetailSerializer(Theme.objects.get(id=create_theme_response.data['id'])).data
