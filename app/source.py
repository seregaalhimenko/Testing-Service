...
def start_quiz(*, request: HttpRequest, pk: int) -> Response:
    ...
    if 'selected_question' in request.session:
        if not (request_is_valid(data=request.data, question_id=request.session['selected_question'].get('id'))):
            question_obj = Question.objects.get(
                id=request.session['selected_question'].get('id'))
    ...
    if not len(request.session['questions_arr']):
        results = InAnswerTrackerSerializer(
            data=request.session['results'], many=True)
        if results.is_valid():
            results.save()
            PassedTests(user=request.user,
                        test=get_object_or_404(Test, id=pk)).save()
    ...
    question_obj = get_object_or_404(
        Question, id=request.session['selected_question'].get('id'))
    ser_question = QuestionShowSerializer(question_obj)
